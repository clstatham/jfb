...
pub fn new(opts: &NewOpts) -> Result<()> {
    ...
    sh.create_dir(&opts.name)?;
    {
        ...
        for (target, target_config) in config.targets.iter() {
            let _guard = sh.push_dir(target);

            for dir in target_config.source_dirs.iter() {
                sh.create_dir(dir)?;
            }

            for dir in target_config.include_dirs.iter() {
                sh.create_dir(dir)?;
            }

            if !opts.bare {
                match (&target_config.target_type, &target_config.language) {
                    ...
                    (TargetType::StaticLibrary, TargetLanguage::C) => {
                        sh.write_file(format!("src/{target}.c"), template_c_library_lib!(target))?;
                        sh.write_file(
                            format!("include/{target}.h"),
                            template_c_library_lib_h!(target),
                        )?;
                    }
                    (TargetType::StaticLibrary, TargetLanguage::Cpp) => {
                        sh.write_file(
                            format!("src/{target}.cpp"),
                            template_cpp_library_lib!(target),
                        )?;
                        sh.write_file(
                            format!("include/{target}.h"),
                            template_cpp_library_lib_h!(target),
                        )?;
                    }
                }
            }
